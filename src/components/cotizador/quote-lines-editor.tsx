...
export function QuoteLinesEditor({
  ...
  const updatePieza = (idx: number, patch: Partial<PiezaLine>) => { const copy = [...piezasLines]; copy[idx] = { ...copy[idx], ...patch }; setPiezasLines(copy); };
  const removePieza = (idx: number) => { const copy = [...piezasLines]; copy.splice(idx, 1); setPiezasLines(copy); };

  const addMaterial = () => setMaterialesLines([...materialesLines, { descripcion: "", qty: 1, unitCost: 0 }]);
  const updateMaterial = (idx: number, patch: Partial<MaterialLine>) => { const copy = [...materialesLines]; copy[idx] = { ...copy[idx], ...patch }; setMaterialesLines(copy); };
  const removeMaterial = (idx: number) => { const copy = [...materialesLines]; copy.splice(idx, 1); setMaterialesLines(copy); };
  ...
  useEffect(() => {
    ...
    fetch('/api/inventory/products')
      .then(r => r.json())
      .then(data => { if (!mounted) return; if (data?.ok && Array.isArray(data.products)) setInventory(data.products); })
      .catch(() => {});
        ...
        {piezasLines.length === 0 ? (
          <div className="text-center py-8">
            ...
            <p className="text-sm text-muted-foreground mb-4">
              No hay piezas detalladas. Puedes ingresar una cantidad manual o agregar piezas específicas.
            </p>
            <Button 
              type="button" 
              variant="outline" 
          ...
          <div className="space-y-3">
            <div className="grid grid-cols-12 gap-3 text-xs font-medium text-muted-foreground px-2 py-1 bg-slate-50 rounded">
              <div className="col-span-7">Producto / Descripción</div>
      ...
      <Card className="p-6 space-y-4">
        ...
        {materialesLines.length === 0 ? (
          <div className="text-center py-8">
            ...
            <p className="text-sm text-muted-foreground mb-4">
              No hay materiales detallados. Puedes ingresar el costo total manual o agregar materiales específicos.
            </p>
            <Button 
              type="button" 
              variant="outline" 
          ...
          <div className="space-y-3">
            <div className="grid grid-cols-12 gap-3 text-xs font-medium text-muted-foreground px-2 py-1 bg-slate-50 rounded">
              ...
              <div className="col-span-3">Costo Unit. ({currency})</div>
            ...
            <div className="mt-4 p-3 bg-green-50 rounded-lg border border-green-200">
              <div className="flex justify-between items-center text-sm">
                ...
                <span className="text-green-900 font-bold text-lg">{totalMaterials.toFixed(2)} {currency}</span>
              </div>
            </div>
          </div>
        )}
