...
const toNum = (v: unknown) => (v == null ? 0 : Number(typeof v === "object" && v ? (v as { toString: () => string }).toString() : v));
...
export const getOTsCached = cache(
  async () => {
    ...
    const skus = Array.from(new Set(ots.flatMap(o => o.materiales.map(m => m.productoId))));
    const sums = skus.length
      ? await prisma.movimiento.groupBy({ by: ["productoId"], where: { productoId: { in: skus } }, _sum: { cantidad: true } })
      : [];
